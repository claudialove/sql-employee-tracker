...
function start() {
    inquirer.prompt({
        ...
        choices: ["VIEW", "ADD", "DELETE", "FINISH"]
    })
        .then(function (answer) {
            ...
            if (answer.activities === "VIEW") {
                ...
                console.log("ok, let's delete a row from our table... note the ID for the row you would like to delete");
                deleteEmployee();
            } else {
                finish();
            }
    ...
    inquirer.prompt([
        {
            type: "input",
            name: "id",
            message: "Enter id for row to delete"
        }
    ]).then(response => {
        connection.query(
